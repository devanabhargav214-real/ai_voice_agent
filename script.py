...
st.write("ఇక్కడ మీరు కేవలం టాపిక్ ఇస్తే కథ జనరేట్ అవుతుంది, నాచురల్ వాయిస్ ఓవర్ పొందవచ్చు మరియు కార్టూన్ లేదా నార్మల్ HD ఇమేజ్స్ (9:16 / 16:9) క్రియేట్ చేయవచ్చు.")

# మూడు ప్రధాన ట్యాబ్స్
tab1, tab2, tab3 = st.tabs([
    "📝 1. AI స్టోరీ జనరేటర్ (Topic)", 
    "🎙️ 2. నాచురల్ AI వాయిస్ ఓవర్", 
    "🎨 3. HD సీన్ ఇమేజ్ జనరేటర్ (Cartoon/Normal)"
])
...
with tab3:
    ...
    with col_style:
        # --- కొత్త ఫీచర్: ఇమేజ్ స్టైల్ సెలెక్షన్ ---
        st.write("#### 🎭 ఇమేజ్ స్టైల్ ఎంచుకోండి:")
        style_option = st.radio(
            "ఫోటో రకం (Image Style):",
            ("కార్టూన్ స్టైల్ (Cartoon/3D Animation style for Kids)", "నార్మల్ రియలిస్టిక్ స్టైల్ (Photorealistic/Cinematic)"),
            index=0
        )
    ...
    # స్టైల్ కాన్ఫిగరేషన్ ప్రాంప్ట్ టెక్స్ట్
    if "కార్టూన్" in style_option:
        style_prompt_addition = "in a beautiful 3D cartoon animation style, Pixar style, cute characters, bright colors, vivid, children's book illustration"
    else:
        style_prompt_addition = "photorealistic, 8k resolution, cinematic lighting, highly detailed masterpiece, realistic textures, real-life look"
    ...
    if st.button("HD సీన్ ఇమేజెస్ జనరేట్ చేయి 🎨"):
        if telugu_script.strip() == "":
            st.warning("దయచేసి కనీసం ఒక తెలుగు సీన్ అయినాタイプ చేయండి!")
        else:
            ...
            for i, t_scene in enumerate(telugu_scenes):
                ...
                with st.spinner(f"సీన్ {i+1} HD ఫోటో తయారవుతోంది... దయచేసి ఆగండి..."):
                    try:
                        # యూజర్ ఎంచుకున్న కార్టూన్ లేదా నార్మల్ స్టైల్‌ని ఇక్కడ ఇంగ్లీష్ ప్రాంప్ట్ కి కలుపుతున్నాం
                        translate_prompt = (
                            f"Convert this Telugu scene into a highly detailed English image generation prompt. "
                            f"The style of the image must be {style_prompt_addition}. "
